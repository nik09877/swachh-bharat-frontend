...
const fetchPickupLocation = async (userId, token) => {
  try {
    ...
    console.log('pickupLocationService:fetchPickupLocation() Success: ', data);
    return data;
  } catch (error) {
    console.error(
...
const fetchPickupLocationByCities = async (cities, token) => {
  try {
    ...
    const { data } = await axios.get(
      `/api/pickup-location/cities?cities=${cities.toString()}`,
      config
    );
    console.log('pickupLocationService:fetchPickupLocation() Success: ', data);
    return data;
...
const uploadImage = async (formData, pickLocId, token) => {
  try {
    const config = {
      headers: { Authorization: `Bearer ${token}` },
    };
    const { data } = await axios.post(
      '/api/pickup-location/upload-image',
      formData,
      config
    );
    console.log('pickupLocationService:uploadImage() Success: ', data);
    return { data: data, isUploaded: true, error: null };
  } catch (error) {
    console.error(
      'pickupLocationService:uploadImage() Error: ',
      error.response.data
    );
    return { data: null, isUploaded: false, error: error.response.data };
  }
};
