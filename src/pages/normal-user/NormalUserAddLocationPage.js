...
import { validateCoordinates } from '../../util/util';
...
const NormalUserAddLocationPage = () => {
  ...
  const [pickupLocations, setPickupLocations] = useState(
    pickupLocationReducer.pickupLocations
  );
  const [oldPickupLocation, setOldPickupLocation] = useState(
    pickupLocations
      ? pickupLocations.filter((loc) => loc.pickLocId == pickLocId)[0]
      : null
  );
  ...
  const [country, setCountry] = useState(
    oldPickupLocation
      ? allCountries.filter((c) => c.name == oldPickupLocation.country)[0]
      : {}
  );
  const [state, setState] = useState(
    oldPickupLocation
      ? getStatesOfCountry(country.id).filter(
          (s) => s.name == oldPickupLocation.state
        )[0]
      : {}
  );
  const [city, setCity] = useState(
    oldPickupLocation
      ? getCitiesOfState(country.id, state.id).filter(
          (c) => c.name == oldPickupLocation.city
        )[0]
      : {}
  );
  const [street, setStreet] = useState(
    oldPickupLocation ? oldPickupLocation.street : ''
  );
  const [landmark, setLandmark] = useState(
    oldPickupLocation ? oldPickupLocation.landmark : ''
  );
        ...
        if (data.type === pickupLocationConstants.ADD_PICKUPLOCATION_SUCCESS) {
          ...
          if (image) {
            ...
            pickupLocationServices
              .uploadImage(formData, data.payload.pickLocId, token)
              .then((response) => {
          ...
          <Form onSubmit={submitHandler} className='addLocation__form'>
            ...
            {!pickLocId && (
              <Form.Group>
                ...
                {image.length ? (
                  <img
                    className='addLocation__form__image'
                    src={image}
                    alt='img'
                  />
                ) : (
